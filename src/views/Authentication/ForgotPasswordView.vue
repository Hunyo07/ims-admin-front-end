  ...
  <div class="flex items-center justify-center min-h-screen bg-gray-100 dark:bg-boxdark">
    <div class="w-full max-w-md p-8 bg-white rounded shadow-md dark:bg-boxdark">
      ...
      <form @submit.prevent="handleSubmit" class="space-y-4">
        ...
        <div>
          ...
          <input
            v-model="email"
            type="email"
            class="w-full border border-stroke rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary"
            required
          />
        </div>
        <button
      ...
      <div class="mt-4 text-xs text-gray-500">
        <p>
          Note: This form uses
          <a href="https://www.emailjs.com/" target="_blank" class="text-primary underline"
            >EmailJS</a
          >. Configure your service/template/user IDs in the code. Your template should use
          <code>{{ reset_link }}</code> for the reset link.
        </p>
      </div>
    </div>
  </div>
