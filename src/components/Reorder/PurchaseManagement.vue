...
const filteredProducts = computed(() => {
  ...
  return products.value.filter((p) => p.supplier && p.supplier._id === newOrder.value.supplierId)
})
...
const exportPurchaseOrderDetailsPDF = () => {
  try {
    ...
    doc.text(
      `Generated on: ${format(now, 'MMM dd, yyyy h:mm a')}`,
    ...
    doc.text(
      `Status: ${
        selectedOrder.value.status.charAt(0).toUpperCase() + selectedOrder.value.status.slice(1)
      }`,
      marginLeft + 5,
      currentY
    )
    ...
    doc.text(
      `Expected Delivery: ${formatDate(selectedOrder.value.expectedDeliveryDate) || 'N/A'}`,
      marginLeft + 5,
      currentY
    )
    currentY += lineHeight - 2
    ...
    if (selectedOrder.value.receivedDate) {
      doc.text(
        `Received: ${formatDate(selectedOrder.value.receivedDate)}`,
        marginLeft + 5,
        currentY
      )
      currentY += lineHeight - 2
    ...
    doc.text(
      `Total Amount: ${formatCurrencyForPDF(selectedOrder.value.totalAmount)}`,
      marginLeft + 5,
      currentY
    )
    ...
    doc.text(
      `Supplier: ${selectedOrder.value.supplier?.name || 'N/A'}`,
      marginLeft + contentWidth / 2 + 10,
      currentY
    )
    currentY += lineHeight - 2

    doc.text(
      `Contact: ${selectedOrder.value.supplier.contactPerson || 'N/A'}`,
      marginLeft + contentWidth / 2 + 10,
      currentY
    )
    currentY += lineHeight - 2
    ...
    doc.text(
      `Phone: ${selectedOrder.value.supplier.phone || 'N/A'}`,
      marginLeft + contentWidth / 2 + 10,
      currentY
    )
    currentY += lineHeight - 2
    ...
    doc.text(
      `Email: ${selectedOrder.value.supplier.email || 'N/A'}`,
      marginLeft + contentWidth / 2 + 10,
      currentY
    )
    ...
    data.push([
      ...
      {
        content: formatCurrencyForPDF(selectedOrder.value.totalAmount),
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      doc.text(
        `Created by: ${
          selectedOrder.value.createdBy?.name || 'Unknown'
        } | Page ${i} of ${pageCount}`,
        pageWidth / 2,
...
const exportPurchaseOrdersPDF = () => {
  try {
    ...
    const totalAmount = paginatedPurchaseOrders.value.reduce(
      (sum, order) => sum + order.totalAmount,
      0
    )
    const pendingOrders = paginatedPurchaseOrders.value.filter(
      (order) => order.status === 'pending'
    ).length
    const receivedOrders = paginatedPurchaseOrders.value.filter(
    ...
    doc.text(
      `Total Amount: ${formatCurrencyForPDF(totalAmount)}`,
    ...
    paginatedPurchaseOrders.value.forEach((order, index) => {
      ...
      const rowData = [
        order.orderNumber,
        order.supplier?.name && order.supplier.name.length > 25
          ? order.supplier.name.substring(0, 25) + '...'
    ...
    for (let i = 1; i <= currentPageNum; i++) {
      ...
      doc.setFont('helvetica', 'italic')
      doc.setTextColor(100, 100, 100)
      doc.text(`Page ${i} of ${currentPageNum}`, pageWidth / 2, doc.internal.pageSize.height - 10, {
        align: 'center'
      })
    }
  ...
  <div>
    ...
    >
      ...
      <div class="p-4">
        ...
        <div
          v-if="totalItems > 0"
          class="flex items-center justify-between px-4 py-3 bg-white dark:bg-boxdark border-t border-stroke dark:border-strokedark"
        >
          <div class="flex items-center space-x-2">
        ...
        <div class="p-6.5">
          <div class="grid grid-cols-2 gap-6 mb-6">
            <div class="col-span-1">
              ...
              <div class="space-y-2">
                <p>
                  <span class="font-medium">Order Number:</span> {{ selectedOrder.orderNumber }}
                </p>
                <p>
            ...
            <div class="col-span-1">
              ...
              <div class="space-y-2">
                ...
                <p>
                  ...
                  {{ selectedOrder.supplier.phone || 'N/A' }}
                </p>
                <p>
