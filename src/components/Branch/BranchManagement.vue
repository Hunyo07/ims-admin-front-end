...
const handleDeleteBranch = async (branchId) => {
  ...
  if (result.isConfirmed) {
    try {
      ...
      await axios.delete(`http://localhost:5000/api/superadmin/branches/${branchId}`, {
        headers: {
          Authorization: `Bearer ${authStore.token}`
        }
      })
  ...
  try {
    ...
    if (result.isConfirmed) {
      const response = await axios.patch(
        `http://localhost:5000/api/superadmin/branches/${branchId}`,
        {},
        {
          headers: {
            Authorization: `Bearer ${authStore.token}`
...
const handleAddBranch = async () => {
  try {
    ...
    const response = await axios.post(
      'http://localhost:5000/api/superadmin/create-branch',
      newBranch.value,
      {
        headers: {
          Authorization: `Bearer ${authStore.token}`
...
const handleUpdateBranch = async () => {
  try {
    ...
    const response = await axios.put(
      `http://localhost:5000/api/superadmin/update-branch/${editingBranch.value._id}`,
      newBranch.value,
      {
        headers: {
          Authorization: `Bearer ${authStore.token}`
