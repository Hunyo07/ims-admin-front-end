...
const saveReceiptAsPDF = () => {
  ...
  doc.text(
    `Date: ${lastSale.value?.date || new Date().toLocaleString()}`,
  ...
  infoY += 5
  doc.text(
    `Total: ${formatCurrencyForPDF(lastSale.value?.total ?? total.value)}`,
    marginLeft + 5,
    infoY
  )
  infoY += 5
  doc.text(
    `Payment: ${formatCurrencyForPDF(lastSale.value?.paymentAmount ?? paymentAmount.value)}`,
    marginLeft + 5,
    infoY
  )
  infoY += 5
  doc.text(
    `Change: ${formatCurrencyForPDF(lastSale.value?.change ?? change.value)}`,
    marginLeft + 5,
    infoY
  )
  if ((lastSale.value?.deliveryMode ?? deliveryMode.value) === 'delivery') {
    infoY += 5
    doc.text(
      `Delivery Fee: ${formatCurrencyForPDF(lastSale.value?.deliveryFee ?? deliveryFee.value)}`,
      marginLeft + 5,
      infoY
    )
  }
  ...
  doc.setFont('helvetica', 'normal')
  doc.text(
    `Name: ${lastSale.value?.customerName || customerName.value}`,
  ...
  custY += 5
  doc.text(
    `Cashier: ${authStore.user?.firstName || ''} ${authStore.user?.lastName || ''}`.trim(),
    marginLeft + contentWidth / 2 + 10,
    custY
  )
  ...
  autoTable(doc, {
    ...
    foot: [
      ...(isDelivery
        ? [
            [
              ...
              {
                content: formatCurrencyForPDF(lastSale.value?.deliveryFee ?? deliveryFee.value),
                styles: { fontStyle: 'bold' as const, halign: 'right' as const }
              }
            ]
          ]
      ...
      [
        ...
        {
          content: formatCurrencyForPDF(lastSale.value?.total ?? total.value),
          styles: { fontStyle: 'bold' as const, halign: 'right' as const }
        }
      ]
    ],
...
const fetchCustomers = async (query = '') => {
  try {
    ...
    const response = await axios.get(
      `http://localhost:5000/api/customers/pos-search?search=${query}&limit=10`,
      {
        headers: {
          Authorization: `Bearer ${authStore.token}`
        }
...
<template>
  <div>
    ...
    <div class="grid grid-cols-1 gap-6 md:grid-cols-3">
      <!-- Product Selection -->
      <div
        class="md:col-span-2 rounded-sm border border-stroke bg-white p-5 shadow-default dark:border-strokedark dark:bg-boxdark"
      >
        <div class="mb-6">
      ...
      >
        ...
        <div class="space-y-5 mt-6">
          ...
          <div
            ...
            class="space-y-5 pt-2 border-t border-dashed border-stroke dark:border-strokedark"
          >
            <div>
              <label class="mb-2.5 block text-sm font-medium text-black dark:text-white"
    ...
    >
      <div class="bg-white p-6 rounded shadow max-w-md w-full relative">
        <button
          @click="closeReceipt"
          class="absolute top-2 right-2 text-gray-400 hover:text-danger"
        >
          &times;
        </button>
        ...
        <div class="flex justify-center gap-4 mt-4">
          <button @click="printReceipt" class="bg-primary text-white px-4 py-2 rounded">
            Print Receipt
          </button>
          <button @click="saveReceiptAsPDF" class="bg-success text-white px-4 py-2 rounded">
            Save as PDF
          </button>
        </div>
      </div>
