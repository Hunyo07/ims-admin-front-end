...
const fetchSalesStats = async () => {
  try {
    ...
    const response = await axios.get(
      `http://localhost:5000/api/sales/stats?${queryParams.toString()}`,
      {
        headers: {
          Authorization: `Bearer ${authStore.token}`
        }
...
const exportSalesPDF = () => {
  try {
    ...
    if (stats.value) {
      ...
      doc.text(
        `Total Revenue: ${formatCurrencyForPDF(stats.value.overview.totalRevenue)}`,
      ...
      // Row 2
      doc.text(
        `Average Order Value: ${formatCurrencyForPDF(stats.value.overview.averageOrderValue)}`,
        marginLeft + 10,
        currentY
      )
      doc.text(
        `Total Items Sold: ${stats.value.overview.totalItems}`,
        marginLeft + 10 + colWidth,
        currentY
      )
    ...
    if (stats.value?.paymentMethods?.length) {
      ...
      doc.roundedRect(
        ...
        currentY,
        contentWidth,
        10 + stats.value.paymentMethods.length * lineHeight,
        2,
        2,
        'FD'
      )
      ...
      stats.value.paymentMethods.forEach((method) => {
        ...
        doc.text(
          `${label}: ${method.count} transactions (${formatCurrencyForPDF(method.total)})`,
          marginLeft + 10,
          currentY
        )
        currentY += lineHeight
...
const exportSaleDetailsPDF = () => {
  ...
  doc.text(
    `Delivery: ${
      sale.deliveryMode
        ? sale.deliveryMode.charAt(0).toUpperCase() + sale.deliveryMode.slice(1)
        : 'Walk-in'
    }`,
    marginLeft + 5,
    infoY + 2 * lineHeight
  ...
  doc.setFontSize(9)
  doc.setTextColor(100, 100, 100)
  doc.text(`Generated by IMS | ${format(now, 'yyyy-MM-dd HH:mm')}`, pageWidth / 2, 285, {
    align: 'center'
  })
  doc.setTextColor(0, 0, 0)
...
<template>
  <div>
    ...
    <!-- Filters -->
    <div
      class="mt-4 rounded-sm border border-stroke bg-white p-4 shadow-default dark:border-strokedark dark:bg-boxdark md:p-6 xl:p-7.5"
    >
    ...
    >
      ...
      <div v-else>
        ...
        >
          ...
          <div class="col-span-1 flex items-center">
            <button @click="viewSaleDetails(sale)" class="hover:text-primary">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                class="h-5 w-5"
                viewBox="0 0 20 20"
      ...
      <!-- Pagination -->
      <div
        class="flex items-center justify-between border-t border-stroke px-4 py-4 dark:border-strokedark sm:px-6"
      >
        ...
        <div class="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
          <div class="flex items-center">
           ...
           -->
            ...
            <label for="itemsPerPage" class="text-sm text-gray-700 dark:text-gray-400 ml-2"
    ...
    >
      <div class="relative w-full max-w-4xl rounded-lg bg-white p-8 dark:bg-boxdark">
        <div class="mb-6 flex items-center justify-between">
          ...
          <button @click="showSaleDetailsModal = false" class="hover:text-danger">
